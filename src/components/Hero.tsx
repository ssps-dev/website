...
export default function Hero() {
  ...
  return (
    <section
      ref={sectionRef}
      className="relative min-h-[75vh] overflow-hidden bg-primary"
    >
      {/* ── Background Video ── */}
      <motion.div
      ...
      <motion.div
        ...
        className="absolute inset-0 z-1 bg-linear-to-b from-primary/90 via-primary/50 to-primary/95"
      />

      {/* ── Arabesque pattern overlay ── */}
      <div className="absolute inset-0 z-2 arabesque-pattern opacity-30" />

      {/* ── Main Grid Content ── */}
      <motion.div
        style={{ y: contentY }}
        className="relative z-3 mx-auto grid min-h-[75vh] max-w-7xl grid-rows-[1fr_auto] px-6 lg:px-8"
      >
        {/* ── Top: Two-column hero content ── */}
        <div className="grid items-center gap-6 pt-24 pb-6 lg:grid-cols-[1.15fr_1fr] lg:gap-10 lg:pt-16 lg:pb-0">
          {/* Left — Tagline & CTAs */}
          <div>
            {/* Eyebrow */}
            <motion.div
            ...
            >
              ...
              <div className="h-px w-10 bg-gradient-to-l from-accent to-transparent" />
              <span className="text-accent text-lg">✦</span>
            </motion.div>
            ...
            >
              <div className="flex gap-1.5">
                {Array.from({ length: 7 }).map((_, i) => (
                  <span key={i} className="text-[8px] text-accent/60">
                    ★
                  </span>
                ))}
              </div>
            ...
            <motion.p
              ...
              className="mt-3 max-w-md text-justify text-[14px] leading-[1.7] font-light text-white/55"
            >
            ...
            >
              <Link
                href="#services"
                className="group flex items-center gap-3 border border-accent bg-accent px-6 py-3.5 text-[11px] font-semibold tracking-[0.15em] text-primary uppercase transition-all duration-300 hover:bg-accent-light"
              >
                Explore Services
                <ArrowRight
                  size={14}
              ...
              <Link
                href="#contact"
                className="border border-white/20 bg-white/5 px-6 py-3.5 text-[11px] font-semibold tracking-[0.15em] text-white/80 uppercase backdrop-blur-sm transition-all duration-300 hover:border-white/50 hover:bg-white/10 hover:text-white"
              >
                Request Quote
              </Link>
            </motion.div>
            ...
            <div className="divide-y divide-white/10 border-t border-b border-white/10">
              {capabilities.map((cap, i) => (
                <motion.div
                  ...
                  className="group flex items-start gap-4 py-3.5 transition-all duration-300 hover:pl-2 hover:bg-white/[0.03]"
                >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between gap-4">
                      <p className="text-[15px] font-semibold tracking-wide text-white transition-colors duration-300 group-hover:text-accent-light">
                        {cap.title}
                      </p>
                    ...
                    <p className="mt-1.5 text-justify text-[13px] leading-relaxed text-white/55">
                      {cap.desc}
                    </p>
                  </div>
                </motion.div>
      ...
      <div className="relative z-3 mx-auto max-w-7xl px-6 pb-6 lg:hidden">
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          {capabilities.map((cap) => (
            <div
              key={cap.num}
              className="border border-white/10 bg-white/5 p-3.5 backdrop-blur-sm"
            >
              <div className="mb-2 flex items-start justify-between">
                <div className="flex h-10 w-10 items-center justify-center border border-accent/30 bg-accent/10 text-accent">
                  <cap.icon size={17} />
                </div>
                <span className="font-mono text-xl font-light text-accent/30">
                  {cap.num}
                </span>
              </div>
              ...
              <p className="mt-1 text-justify text-[12px] leading-relaxed text-white/55">
                {cap.desc}
              </p>
            </div>
          ))}
